...
export const createSpeakerRecord = async (speaker: Speaker) => {
  const {
    firstName,
    lastName,
    company,
...
export const deleteSpeakerRecord = async (id: number) => {
...
export const getSpeakers = async (attendeeId: string) => {
  //} : Promise<Speaker[]>  {
  try {
    await sleep(1000);
    const speakers = (
...
export const getSpeakerDataById = async (id: number, attendeeId?: string) => {
...
export const updateSpeakerRecord = async (
  speaker: Speaker,
